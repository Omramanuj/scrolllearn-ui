...
export default function LearningCard({ card, index }: LearningCardProps) {
  // Rotate between our vibrant colors based on card index
  ...
  return (
    <div 
      id={`card-${card.id}`}
      className={`h-[85vh] w-full snap-center flex flex-col justify-center p-8 bg-[${color}] relative rounded-3xl shadow-xl my-4 before:absolute before:content-[""] before:w-16 before:h-16 before:top-0 before:left-0 before:border-l-2 before:border-t-2 before:border-[${color}] before:rounded-tl-3xl`}
    >
