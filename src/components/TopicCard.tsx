...
export default function TopicCard({ topic, index  }: TopicCardProps) {
  ...
  return (
    <Link href={`/topics/${topic.id}`} className="block w-full">
      <div 
        className={`relative rounded-3xl p-6 shadow-lg transition-all hover:scale-105 hover:shadow-xl bg-[${color}]`}
      >
        <h3 className="text-2xl font-bold text-white mb-3 drop-shadow-sm">{topic.topic} {index}</h3>
